...
library(ggbiplot)
library(readr)
p_accessory <- read_csv("Nicole's Habitat Data Sets - p_accessory.csv")
p_accessory_matrix = subset(p_accessory, select=c(crtRa, crtRb, crtG, crtX, crtW, crtOa, crtN,crtP, crtQa, crtQb, cruF, crtM),row.names=1)
row.names(p_accessory)=p_accessory$Strain
pca_p_accessory= prcomp(p_accessory_matrix, center= TRUE, scale.= TRUE)

#analyze pca statistics to find proportion of variatiance explained by each gene 
summary(pca_p_accessory)
str(pca_p_accessory)

#make groups based on habitat
habitat= p_accessory$Habitat
...
ggbiplot(pca_p_accessory,ellipse=FALSE,obs.scale = 3,var.scale = 1, groups=habitat) +
...
#png("PCA Plot of Carotenoid BLAST Hit .png", width=1000, height=1000)
ggbiplot(pca_p_accessory,labels= rownames(p_accessory),ellipse=FALSE,obs.scale = 3,var.scale = 1, groups=habitat) +
  scale_colour_manual(name="Habitat", values= c("cornflowerblue", "forest green", "darkorange1", "saddlebrown"))+
  ggtitle("PCA of pHMMer Accessory Enzymes -N")+
  theme_minimal()+
  theme(legend.position = "bottom")
...
p_accessory <- read_csv("Nicole's Habitat Data Sets - p_accessory.csv")
p_accessory_matrix = subset(p_accessory, select=c(crtRa, crtRb, crtG, crtX, crtW, crtOa, crtN,crtP, crtQa, crtQb, cruF, crtM),row.names=1)
nmds_p_accessory_habitat= as.data.frame(p_accessory[,1:2])
#Do you need to transform your data? range should be 0-10 if not, use root function
range((p_accessory_matrix)^0.5) # 0-11 should be 0-10 but lets see if the data looks fine 

#make a distance matrix
p_accessory_matrix_ds=vegdist((p_accessory_matrix), method = 'euclidian')

#Run metaMDS on distance matrix
p_accessory_nms=metaMDS(p_accessory_matrix_ds, distance="euclidian",k=2,trymax=500)
p_accessory_nms
#stress=0.08

#assign colors to habitat
cols <- nmds_p_accessory_habitat$Strain
cols[] <- c("cornflowerblue", "forest green", "darkorange1", "saddlebrown")
#Warning message:
#number of items to replace is not a multiple of replacement length

cols[nmds_p_accessory_habitat$Habitat == c("aqutic")] <- "cornflowerblue"
cols[nmds_p_accessory_habitat$Habitat == c("hydroterrestrial")] <- "forest green"
cols[nmds_accessory_habitat$Habitat == c("soil")] <- "darkorange1"
cols[nmds_accessory_habitat$Habitat == c("subaerial")] <- "saddlebrown"
cols[nmds_accessory_habitat$Habitat == c("NA")] <- "grey"
pairs(character(nrow(nmds_p_accessory_habitat$Strain, col=cols)))
              
habitatinfo= c(rep(p_accessory$Habitat))
```

```{r NMDS Plots, echo= FALSE}
#NMDS Plot With Species Names 
ordiplot(p_accessory_nms, type="n")
orditorp(p_accessory_nms,display="sites",labels=p_accessory$Strain, col=cols ,air=0.00025)
par(xpd=TRUE)
...
scores <-
  scores(p_accessory_nms, display = "sites", "species")
cent <-
  aggregate(scores ~ habitatinfo, data = nmds_p_accessory_habitat, FUN = "mean")
...
ordiplot(p_accessory_nms, choices= c(1,2))
points(p_accessory_nms,display= "sites",pch=21, bg=cols) 
par(xpd=TRUE)
...
#data prepartion 
rownames(p_accessory_matrix) = p_accessory$Strain
str(p_accessory_matrix)
nmds_p_accessory_habitat= as.data.frame(p_accessory[,1:2])
str(nmds_p_accessory_habitat)
...
#Run PERMANOVA
pmv_p_accessory = adonis(p_accessory_matrix ~ habitat, data = nmds_p_accessory_habitat, permutations = 999, method = 'bray')
pmv_p_accessory
...
bd_p_accessory = betadisper(p_accessory_matrix_ds, p_accessory$Habitat)
boxplot(bd_p_accessory)
anova(bd_p_accessory) 
#F-Test: pvalue 0.53
...
simp_p_accessory= simper(p_accessory_matrix, group= p_accessory$Habitat)
summary(simp_p_accessory)
