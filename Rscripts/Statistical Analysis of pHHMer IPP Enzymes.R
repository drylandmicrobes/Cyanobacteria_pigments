---
title: "Statistical Analysis of pHHMer IPP Enzymes"
...
library(ggbiplot)
library(readr)
phmmer_IPP <- read_csv("Cyano Carotenoids pHMMER Results - phmmer_IPP.csv")
#organize data to just raw hits and strain names 
row.names(phmmer_IPP) = phmmer_IPP$Strain
phmmer_IPP_matrix <- data.matrix(phmmer_IPP[c(3:9)])
range(phmmer_IPP_matrix) #0 10

#analyze pca statistics to find proportion of variatiance explained by each gene 
pca_phmmer_IPP= prcomp(phmmer_IPP_matrix, center= TRUE, scale.= TRUE)
summary(pca_phmmer_IPP)
str(pca_phmmer_IPP)
#


#make groups based on habitat
habitat= phmmer_IPP$Habitat
...
ggbiplot(pca_phmmer_IPP,ellipse=TRUE,obs.scale = 3,var.scale = 1, groups=habitat) +
  ...
  ggtitle("PCA of pHMMer IPP Enzymes")+
  theme_minimal()+
...
pairs(character(nrow(nmds_pHMMER_habitat$Strain, col=cols)
                ...
                #data prepartion 
                rownames(phmmer_IPP_matrix) = phmmer_IPP$Strain
                str(phmmer_IPP_matrix)
                nmds_phmmer_IPP_habitat= as.data.frame(phmmer_IPP[,1:2])
                str(nmds_phmmer_IPP_habitat)
                
                #make a distance matrix
                phmmer_IPP_matrix_ds=vegdist(phmmer_IPP_matrix, method = 'euclidian')
                
                #Run metaMDS on distance matrix
                phmmer_IPP_nms=metaMDS(phmmer_IPP_matrix_ds, distance="euclidian",k=2,trymax=500)
                phmmer_IPP_nms
                #stress=0.045
                
                #Run PERMANOVA
                pmv_phmmer_IPP = adonis(phmmer_IPP_matrix ~ habitat, data = nmds_phmmer_IPP_habitat, permutations = 999, method = 'euclidean')
                pmv_phmmer_IPP
                ...
                #between their habitats (p-value:0.021).
                ...
                densityplot(permustats(pmv_phmmer_IPP))
                
                #Distance Based Dispersion test 
                bd_phmmer_IPP = betadisper(phmmer_IPP_matrix_ds, phmmer_IPP$Habitat)
                boxplot(bd_phmmer_IPP)
                anova(bd_phmmer_IPP) 
                #F-Test: pvalue 0.05076
                permutest(bd_phmmer_IPP)
                #permutation test: pvalue 0.068
                ...
                simp_pHMMER_IPP= simper(pHMMER_matrix, group= pHMMER$Habitat)
                summary(simp_pHMMER)
